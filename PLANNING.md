...
開発の時系列タスク・進捗管理は **[ROADMAP.md](./ROADMAP.md)** に分離しました。本ドキュメントは設計・要件に集中し、ロードマップは「コア機能 / 配布 / ユーザビリティ / 音質 / 品質運用」の5トラックそれぞれにステージ(A/B/C...)を持たせて並行管理しています。
...
```
ezvcc/
├── PLANNING.md              # 本ファイル(設計・要件)
├── ROADMAP.md               # 開発ロードマップ(トラック別ステージ管理)
├── README.md                # 利用者向け説明
├── DEVELOPMENT.md           # 開発者向けセットアップ・ビルド・デバッグ
├── ezvcc.sln
├── src/
│   ├── ezvcc.App/           # WPFアプリ本体
│   │   ├── Views/
